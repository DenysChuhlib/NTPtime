...
enum NTP_Status : uint8_t {
    ...
    NTP_NOT_CONNECTED_Ethernet      // 8 - не підключений Ethernet
};
...
public:
    NTPtime(int8_t gmt = 0, int16_t gmtM = 0) {
        setTimeZone(gmt, gmtM);
        startTime();
        
        asyncMode(true);
        _ntp_stat = NTP_NOT_STARTED;
    }
    ...
    bool tick(uint32_t prd = 0) {
        if (prd) {
            prd = constrain(prd, 60, 86400);
            if (msFromUpdate() >= prd  * 1000) setTimeStat(UNIX_NOT_SYNCHRONIZED);
...
private:
    ...
    uint8_t requestTime() {
        uint8_t buf[48];
        #ifndef NTPtimeEthernet
        if (!WiFi.isConnected()) return NTP_NOT_CONNECTED_WIFI;
        #else
        if (Ethernet.linkStatus() == LinkOFF || Ethernet.hardwareStatus() == EthernetNoHardware) return NTP_NOT_CONNECTED_Ethernet;
        #endif
        if (!_send_pack) {//перевіряємо чи не був відправлений запит
            ...
            if (!udp.beginPacket(_host.c_str(), _port)) return NTP_NOT_CONNECTED_TO_SERVER;
            udp.write(buf, 48);
            if (!udp.endPacket()) return NTP_NOT_SENT_PACKET;
            _way = millis();
            _send_pack = true;
        }
        if (_send_pack) {//перевіряємо чи був ли відправлений запит
            if (udp.parsePacket() != 48 || udp.remotePort() != _port) {
                ...
                udp.read(buf, 48);                  // читаємо
                {
                    ...
                    if (unix < TimeFunc.getUnixGMTCompiled(getTimeZoneM()) - 86400UL) {  // некоректний час (віднімання доби для коректного переходу між часовими поясами)
                        _send_pack = false;
                        _ping = -1;
                        return NTP_REPLY_ERROR;
                    }
